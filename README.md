...
Results rebuilt from zero. One fold, one story. The results background
now plays its own dedicated checkout footage (vid-checkout.mp4), so the
card and the page never show the same clip twice.
...
## Cache fix
All assets renamed (vid-*.mp4 / img-*.jpg (incl. vid-checkout for the results background)). The old deploy's cached
